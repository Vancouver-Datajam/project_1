...
## Hot spots for regions with high amount of items
...
top_5 <-
  ...
  summarise(Totalltems_EventRecord = sum(Totalltems_EventRecord, na.rm = TRUE)) %>%
  slice_max(Totalltems_EventRecord, n = 5)


```
- Show the indivudal spots for the top 5 regions: `r top_5$SubCountry_L2_FromSource`
- To observe the hot spot for the top 5 regions
...
map_spot <- function(region){
  ...
  map_region <-
    ...
    addMarkers(
      ...
      lng = ~ X,
      lat = ~ Y,
      label = ~ paste0(DateOriginal, ": ", Totalltems_EventRecord, " items"),
